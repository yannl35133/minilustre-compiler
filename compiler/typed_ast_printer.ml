...
let rec print_list print_elt sep fmt l = match l with
  | []     -> ()
  | [x]    -> print_elt fmt x
  | h :: t -> fprintf fmt "%a%s@ %a" print_elt h sep (print_list print_elt sep) t

let rec print_list_eol print_elt sep fmt l = match l with
  | []     -> ()
  | [x]    -> fprintf fmt "%a%s"      print_elt x sep
  | h :: t -> fprintf fmt "%a%s@\n%a" print_elt h sep (print_list_eol print_elt sep) t
